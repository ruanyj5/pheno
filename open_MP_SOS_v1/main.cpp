...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel for num_threads(num_threads) private(j,i,k,l,m,TEMP46NDVI, EXT09Data, EXT09DMSK2, LocMaxima,x,y,param,paramfit,pred_f)
    for ( j = 0; j < cols; ++j) {
        ...
        //NDVI46 to 365
        NDVI46to365(TEMP46NDVI, EXT09Data, EXT09DMSK2, &LocMaxima);
...
//    #pragma omp parallel for private(j)
    for (int j=0;j<cols;j++) {
        SOS6method[0*cols+j]=(short int)Date1[j];
        SOS6method[1*cols+j]=(short int)Date2[j];
        SOS6method[2*cols+j]=(short int)Date3[j];
        SOS6method[3*cols+j]=(short int)Date4[j];
        SOS6method[4*cols+j]=(short int)Date5[j];
        SOS6method[5*cols+j]=(short int)Date6[j];
    }
